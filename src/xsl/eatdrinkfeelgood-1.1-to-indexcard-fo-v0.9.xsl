...
<!-- ====================================================================== 
     ...
     0.9   Decemeber 30, 2002

           - Initial public release

     ====================================================================== 
     Bugs, todos and gotchas
     ...
     - If your 'history' element is very long, it may result in pushing the
       ingredients list on to the second page. I will try to address this
       with judicious use of the 'float' property. Ultimately, the ingredients
       list should always begin on the right hand side of the first page and
       any extra history content should flow on to the second.

     - Proper footnote formatting in Fop is often sketchy when used in conjunction
       with footers (region-after) so they are only used on the first page.
     ...
     - Fop doesn't really wrap list labels so I've included an ugly hack in
       the 'Authors' template. Author elements should probably be rendered as
       footnotes anyway (see above.)

     - Templates shared with other eatdrinkfeelgood 1.1 stylesheets should be
       moved into a 'shared' XSL document and included.

     - The various 'fo:*' templates should be renamed accordingly where they 
       are really the equivalent of a CSS class.
...
<!-- ====================================================================== 
     ...
     <xsl:key name = "authors" match = "e:author/e:extref" use = "@xlink:title" />
     ...
     <xsl:template match = "e:note">
      <fo:block>
       ...
       <fo:block>
        ...
        <xsl:apply-templates select = "e:author" />, <xsl:apply-templates select = "dc:date" />
       </fo:block>
      </fo:block>  
     </xsl:template>
...
<!-- ======================================================================
     ...
     <xsl:template match = "e:publication">
      ...
      <xsl:if test = "string(@xlink:href) or string(@isbn)">

      <xsl:variable name = "fn">*</xsl:variable>

       <fo:footnote>
        <fo:inline>
         <xsl:call-template name = "fo:footnote-attrs" />
         <xsl:value-of select = "$fn" />
        </fo:inline>   

        <fo:footnote-body>
         <fo:list-block>
          <xsl:attribute name = "provisional-distance-between-starts">8pt</xsl:attribute>
         <xsl:if test = "string(@isbn)">
          <fo:list-item>
           <fo:list-item-label>
            <fo:block>
             <xsl:call-template name = "fo:footnote-attrs" />
             <fo:inline><xsl:value-of select = "$fn" /></fo:inline>
            </fo:block>
           </fo:list-item-label>
          <fo:list-item-body start-indent = "body-start()">
           <fo:block>
            <xsl:call-template name = "fo:footnote-body-attrs" />
     ...
     <xsl:template name = "Authors">
      <fo:block>
       ...
       <fo:list-block>
        ...
        <xsl:for-each select = "*//e:author/e:extref[count(.|key('authors', @xlink:title)[1]) = 1]">
        <fo:list-item>
          <fo:list-item-label>
           <fo:block>
            <!-- 20021230
                 Fop won't/can't wrap labels properly so this is
                 an unsatisfying hack until I figure out what to
                 do about it.
                 -->
         ...
         <fo:list-item-body>
          ...
          <fo:block>
           ...
           <xsl:if test = "string(@xlink:title) != '' and string(@xlink:href) != ''">
            <xsl:text>, </xsl:text>               
           </xsl:if>
...
<!-- ====================================================================== 
     ...
     <xsl:template name = "fo:footnote-attrs">
      <xsl:attribute name = "font-family">sans-serif</xsl:attribute>
      <xsl:attribute name = "font-size">6pt</xsl:attribute>
      <xsl:attribute name = "baseline-shift">super</xsl:attribute>
     </xsl:template>

     <xsl:template name = "fo:footnote-body-attrs">
      <xsl:attribute name = "font-family">sans-serif</xsl:attribute>
      <xsl:attribute name = "font-size">6pt</xsl:attribute>
     </xsl:template>
